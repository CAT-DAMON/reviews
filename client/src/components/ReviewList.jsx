...
class ReviewList extends React.Component {
  ...
  getItemReviews(sort) {
    $.get(`http://54.219.72.14:3001/api-items/${this.state.storeId}/${this.state.id}/${this.state.sort}`)
      .done((reviews) => {
        console.log(reviews)
  ...
  getShopReviews(sort) {
    $.get(`http://54.219.72.14:3001/api-store/${this.state.storeId}/${this.state.sort}`)
      .done((reviews) => {
  ...
  clickHelpful(e) {
    ...
    $.ajax({
      url: `http://54.219.72.14:3001/api-helpful/${this.state.storeId}/${e.target.id}`,
      method: 'PATCH'
    })
    .done((changed) => {
